...
const Tables = ({ headers, items, primaryId, isCentered = false }) => {
  ...
  const populateItem = (i, h, item) => {
    const className = `p-3 font-semibold ${
      h.customValueClass ? h.customValueClass : ""
      ...
      <div className="flex justify-end gap-2 mt-4">
        ...
        {pageNumbers.map((number, index) => (
          <button
            ...
            className={`py-2 px-4 rounded border border-[#3A3A3A] hover:bg-[#3A3A3A] hover:text-white ${
              currentPage === number
